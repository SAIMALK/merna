...
const PestDetailsScreen = ({ route }) => {
  const { pestId } = route.params;
  ...
  const fetchingPestDetails = async () => {
    try {
      const response = await fetch(`${ADDRESS}/api/pests/${pestId}`);
  ...
  if (!data) {
    return <View style={styles.empty}><Text>No Pest details available</Text></View>;
  }
